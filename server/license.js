 ...
 */
const getLicense = () => {
  const template = fs.readFileSync('./license/template/1.1/lic.template', 'utf8');
  ...
  try {
    ...
    if (data.valid) {
      ...
      appLicense.scenarios = parseInt(data.data.scenarios);
      appLicense.valid = false;
      appLicense.errorMessage = 'Invalid License Key, please contact your Administrator';
    }
  } catch (err) {
    logger.error(err);
    appLicense.valid = false;
    appLicense.errorMessage = 'License Key Not Found, please contact your Administrator';
  }
  return appLicense;
};
